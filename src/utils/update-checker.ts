...
export class UpdateChecker {
		...
		try {
			...
			if (!currentVersion) {
				this.outputChannel.appendLine(
					"[UpdateChecker] ERROR: Could not get current extension version",
				);
				return;
			}

			this.outputChannel.appendLine(
				`[UpdateChecker] Checking for updates... (current: ${currentVersion})`,
			);
			...
			const skipVersion = this.context.globalState.get<string>(
			...
			this.outputChannel.appendLine(
				`[UpdateChecker] ERROR: Failed to check for updates: ${error}`,
			);
		}
	}
	...
	private async fetchLatestRelease(): Promise<any> {
		try {
			...
			if (!response.ok) {
				this.outputChannel.appendLine(
					`[UpdateChecker] GitHub API returned ${response.status}: ${response.statusText}`,
				);
				return null;
			}
			...
			this.outputChannel.appendLine(
				`[UpdateChecker] Latest release: ${release?.tag_name || "unknown"}`,
			);
			return release;
		} catch (error) {
			this.outputChannel.appendLine(
				`[UpdateChecker] ERROR: Failed to fetch latest release: ${error}`,
			);
			return null;
		}
	}
	...
	private showUpdateNotification(
		...
		vscode.window
			.showInformationMessage(message, "View Changelog", "Skip")
