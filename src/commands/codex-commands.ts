...
export class CodexCommands {
   ...
   */
  static registerCommands(context: vscode.ExtensionContext, codexProvider: CodexProvider): void {
    const commands = new CodexCommands(codexProvider);

    const disposables = [
      vscode.commands.registerCommand('kiro.checkCodexAvailability', () => commands.checkCodexAvailability()),
      vscode.commands.registerCommand('kiro.showCodexSetupGuide', () => commands.showCodexSetupGuide()),
      vscode.commands.registerCommand('kiro.testCodexConnection', () => commands.testCodexConnection())
    ];
  ...
  async checkCodexAvailability(): Promise<void> {
    try {
      vscode.window.withProgress({
        ...
        progress.report({ increment: 0, message: 'Detecting Codex CLI installation...' });

        const result = await this.codexProvider.getCodexAvailabilityStatus();

        progress.report({ increment: 50, message: 'Checking version compatibility...' });
        ...
        if (result.isAvailable) {
          vscode.window.showInformationMessage(
            ...
            if (action === 'Show Configuration') {
              vscode.commands.executeCommand('workbench.action.openSettings', 'kiro.codex');
            }
          });
        } else {
      ...
      vscode.window.showErrorMessage(`Failed to check Codex availability: ${errorMessage}`);
    }
  }
  ...
  async testCodexConnection(): Promise<void> {
    try {
      vscode.window.withProgress({
        ...
        progress.report({ increment: 0, message: 'Checking availability...' });

        const isReady = await this.codexProvider.isCodexReady();

        if (!isReady) {
          throw new Error('Codex CLI is not available. Please check your installation.');
        }

        progress.report({ increment: 30, message: 'Testing basic functionality...' });
        ...
        if (testResult.exitCode === 0) {
          ...
          throw new Error(`Codex CLI test failed with exit code ${testResult.exitCode}: ${testResult.error}`);
        }
      });
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
...
- Include JSDoc comments`;
      ...
      await this.codexProvider.invokeCodexSplitView(samplePrompt, 'Codex Sample Test');
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      vscode.window.showErrorMessage(`Failed to run sample command: ${errorMessage}`);
    }
  }
}
