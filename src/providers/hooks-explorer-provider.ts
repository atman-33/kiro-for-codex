...
export class HooksExplorerProvider
	implements vscode.TreeDataProvider<HookItem>
{
	private _onDidChangeTreeData: vscode.EventEmitter<
		HookItem | undefined | null | void
	> = new vscode.EventEmitter<HookItem | undefined | null | void>();
	...
	private async getCodexHooks(): Promise<
		{ name: string; enabled: boolean; config: any; configPath: string }[]
	> {
		const hooks: {
			name: string;
			enabled: boolean;
		...
		try {
			...
			const codexConfigPath = path.join(homeDir, ".codex", "settings.json");
