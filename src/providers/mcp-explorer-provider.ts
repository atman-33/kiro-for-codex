...
export class MCPExplorerProvider implements vscode.TreeDataProvider<MCPItem> {
	private _onDidChangeTreeData: vscode.EventEmitter<
		MCPItem | undefined | null | void
	> = new vscode.EventEmitter<MCPItem | undefined | null | void>();
	...
	private async loadServerDetails(name: string) {
		try {
			...
			const { stdout, stderr } = await execAsync(`codex mcp get ${name}`, {
				cwd,
			});

			if (stderr) {
				this.outputChannel.appendLine(
					`Error getting details for ${name}: ${stderr}`,
				);
			...
			this.outputChannel.appendLine(
				`Failed to get details for ${name}: ${error}`,
			);
