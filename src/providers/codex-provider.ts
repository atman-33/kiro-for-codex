...
export class CodexProvider {
  ...
  async checkCodexInstallationAndCompatibility(): Promise<CodexAvailabilityResult> {
    ...
    try {
      ...
      if (versionResult.exitCode === 0) {
        ...
        const notFound = this.isCliNotFound(versionResult.exitCode, versionResult.output, versionResult.error);
        if (notFound) {
   ...
   */
  private isCliNotFound(exitCode: number, stdout?: string, stderr?: string): boolean {
    const out = `${stdout || ''}\n${stderr || ''}`;
    ...
    if (process.platform === 'win32' && replacementCharCount >= 5 && lower.includes("codex")) {
      return true;
    }
          ...
          if (hasBuildArgs) {
            ...
            if (process.platform === 'win32') {
              ...
              for (let i = 0; i < args.length; i++) {
                const arg = args[i];
                if (arg === '-a') { i++; continue; } // skip approval mode for exec
                if (arg === '--full-auto') { continue; }
              ...
              // Non-Windows: pass prompt as a single argv argument
              result = await Promise.race([
                this.processManager.executeCommandArgs(
                  this.codexConfig.codexPath,
                  [...args, prompt],
      ...
      async () => {
        ...
        try {
          ...
          if (process.platform === 'win32') {
            ...
            const modelPart = this.codexConfig.defaultModel ? `-m "${this.codexConfig.defaultModel}"` : '';
          ...
          const terminal = this.processManager.createTerminal(command, {
            ...
            ...(process.platform === 'win32' ? { shellPath: 'powershell.exe', shellArgs: ['-NoLogo', '-NoProfile'] } : {})
          } as any);
