...
import type { AgentInfo, AgentManager } from "../features/agents/agent-manager";

export class AgentsExplorerProvider
	implements vscode.TreeDataProvider<AgentItem>
{
	...
	private setupFileWatchers(): void {
		...
		if (workspaceFolder) {
			...
			this.fileWatcher =
				vscode.workspace.createFileSystemWatcher(codexAgentsPattern);
			const codexWatcher =
				vscode.workspace.createFileSystemWatcher(codexAgentsPattern);
		...
		const userCodexAgentsPath = path.join(
			require("os").homedir(),
			".codex/agents",
		);
		...
		try {
			this.userFileWatcher = vscode.workspace.createFileSystemWatcher(
				userCodexAgentsPattern,
			);
			const userCodexWatcher = vscode.workspace.createFileSystemWatcher(
				userCodexAgentsPattern,
			);
			...
			this.outputChannel.appendLine(
				`[AgentsExplorer] Failed to watch user agents directory: ${error}`,
			);
		}
	}
...
class AgentItem extends vscode.TreeItem {
	constructor(
		...
		if (contextValue === "agent-loading") {
			...
			this.description = agentInfo.tools
