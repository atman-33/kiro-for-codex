...
export class PromptsExplorerProvider implements vscode.TreeDataProvider<PromptItem> {
    ...
    constructor(private context: vscode.ExtensionContext, codexProvider: CodexProvider) {
    ...
    async getChildren(element?: PromptItem): Promise<PromptItem[]> {
        ...
        if (!element) {
            ...
            if (this.isLoading) {
                items.push(new PromptItem('Loading prompts...', vscode.TreeItemCollapsibleState.None, 'prompts-loading'));
                return items;
            }
            ...
            if (files.length === 0) {
                items.push(new PromptItem('No prompts found', vscode.TreeItemCollapsibleState.None, 'prompts-empty'));
                return items;
            }

            return files.map(fp => new PromptItem(
                path.basename(fp),
    ...
    private async readMarkdownFiles(dir: string): Promise<string[]> {
        ...
        try {
            ...
            for (const [name, type] of entries) {
                ...
                if (type === vscode.FileType.File && name.endsWith('.md')) out.push(full);
