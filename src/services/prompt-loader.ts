...
export class PromptLoader {
  ...
  public renderPrompt(promptId: string, variables: Record<string, any> = {}): string {
    const resolvedId = this.resolveTemplateId(promptId);
  ...
  /**
   * Resolve requested template ID. Optimization logic has been removed.
   */
  private resolveTemplateId(requestedId: string): string {
    return requestedId;
  }
