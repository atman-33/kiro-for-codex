...
export function Composer({ onSend, isRunning, onStop }: { onSend?: (text: string, id: string) => void; isRunning?: boolean; onStop?: () => void; }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 8, width: '100%', boxSizing: 'border-box', minWidth: 0 }}>
      <input
        value={text}
        ...
        disabled={!!isRunning}
        style={{ flex: 1, padding: '8px 10px', borderRadius: 6, border: '1px solid var(--vscode-editorWidget-border, #555)', minWidth: 0 }}
      />
      {isRunning ? (
